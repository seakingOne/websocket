...
import com.ynhuang.websocket.chat.util.Base64Util;
 ...
 *  5、确定当前连接设置的最大连接数
 *
 *  5.1修改catalina.sh或者catalina.bat文件，加上这句话（JAVA_OPTS="-server -Xms2048m -Xmx2048m -Xmn512m"）。
 *
 *  5.2修改conf下server.xml,    
 ...
 *  6、数据传输的粘包问题？
 *
 *  7、websocket搭建集群环境、session的共享问题？
 *
 */
@Component
@Slf4j
...
public class WebSocketServer {
  ...
  */
    ...
    public void onError(@PathParam("userId") String userId,
                        Throwable throwable,
                        Session session) {
        log.info("[WebSocketServer] Connection Exception : userId = "+ userId
